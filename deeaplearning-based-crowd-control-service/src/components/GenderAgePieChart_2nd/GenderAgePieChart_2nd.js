...
const GenderAgePieChart = ( {setSelectedData} ) => {
  ...
  // 랜덤 값 생성 함수
  const getMRanVal = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
  const getFRanVal = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
  ...
  }, [selectedGender, mKids, mTeen, mAdult, mMid, mEld, fKids, fTeen, fAdult, fMid, fEld, setSelectedData]);
