...
const AnalyzeInfo = {
  ...
  getWeekAvg: (user_id, callback) => {
    ...
    AVG(CASE 
        ...
        THEN a.population 
    END) AS this_week_avg_population,
    AVG(CASE 
        WHEN a.time BETWEEN CURDATE() - INTERVAL 14 DAY AND CURDATE() - INTERVAL 7 DAY 
        THEN a.population 
    END) AS last_week_avg_population,
    AVG(CASE 
        WHEN a.time BETWEEN CURDATE() - INTERVAL 1 MONTH AND NOW() 
        THEN a.population 
    END) AS last_month_avg_population
  FROM analyze_info a
  JOIN zone z ON a.zone_id = z.zone_id
  JOIN exhibition e ON z.user_id = e.user_id AND z.exhb_id = e.exhb_id
